...
class TelegramCollector:
    ...
    async def start_collection(self, automation_id: int):
        ...
        try:
            # Busca a automação
            automation = self.db.query(AutomationModel).filter(AutomationModel.id == automation_id).first()
            if not automation:
                logger.error(f"Automação {automation_id} não encontrada")
                return
        
            # Resolve a sessão e credenciais da API
            session_obj = self.db.query(UserSession).filter(UserSession.id == automation.session_id).first()
            if not session_obj:
                logger.error(f"Sessão vinculada à automação {automation_id} não encontrada")
                return

            if not session_obj.api_id or not session_obj.api_hash:
                logger.error("Credenciais da API ausentes na sessão. Recrie a sessão com API_ID/API_HASH.")
                return
    ...
    async def process_message(self, message: Message, automation_id: int):
        ...
        try:
            ...
            existing = self.db.query(CollectedMedia).filter(
